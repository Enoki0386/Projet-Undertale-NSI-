...
class TitleScreen:
    """
    Affiche l'écran d'accueil et retourne jouer ou quitter
    """
    ...
    def __init__(self, screen: pygame.Surface):
        ...
        total_h  = 3 * BUTTON_H + 2 * BUTTON_GAP
        top_y    = self.h - 110 - total_h
        cx       = self.w // 2 - BUTTON_W // 2

        self.btn_play  = pygame.Rect(cx, top_y,                               BUTTON_W, BUTTON_H)
        self.btn_saves = pygame.Rect(cx, top_y + BUTTON_H + BUTTON_GAP,       BUTTON_W, BUTTON_H)
        self.btn_quit  = pygame.Rect(cx, top_y + 2 * (BUTTON_H + BUTTON_GAP), BUTTON_W, BUTTON_H)
    ...
    def draw_title(self):
        ...
        # Sous-titre
        sub  = self.font_sub.render('Projet pygame', True, (100, 100, 100))
        self.screen.blit(sub, (self.w // 2 - sub.get_width() // 2, ty + surf.get_height() + 8))


    def draw_buttons(self, mx: int, my: int):
        self.btn(self.btn_play,  'JOUER',        mx, my, disabled=False)
        self.btn(self.btn_saves, 'SAVES',  mx, my, disabled=True)
